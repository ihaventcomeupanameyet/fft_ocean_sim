    ...
    const R = i.__csm.prevOnBeforeCompile, o = (s, n, f) => {
      ...
      if (n) {
        ...
        if (r2 !== -1) {
          l2 = n.slice(0, r2);
          let c2 = 0, g = -1;
          for (let M = r2; M < n.length; M++)
            if (n[M] === "{" && c2++, n[M] === "}" && (c2--, c2 === 0)) {
              g = M;
              break;
            }
          if (g !== -1) {
            const M = n.slice(r2, g + 1);
...
` + t), s.includes("//~CSM_DEFAULTS")) {
        ...
        const r2 = s.lastIndexOf("//~CSM_MAIN_END");
        if (r2 !== -1) {
          const c2 = `
            ${t ? `${t}` : ""}
            //~CSM_MAIN_END
          `;
          s = s.slice(0, r2) + c2 + s.slice(r2);
        }
      } else {
    ...
    i.onBeforeCompile = (s, n) => {
      R == null || R(s, n);
      const f = p2 || {}, t = i.type, l2 = t ? `#define IS_${t.toUpperCase()};
` : `#define IS_UNKNOWN;
`;
      s.vertexShader = l2 + `#define IS_VERTEX
` + s.vertexShader, s.fragmentShader = l2 + `#define IS_FRAGMENT
` + s.fragmentShader;
      const T = (r2) => {
        for (const c2 in r2) {
          const g = c2 === "*" || S && S.includes(c2);
